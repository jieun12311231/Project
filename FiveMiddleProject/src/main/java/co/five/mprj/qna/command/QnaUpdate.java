...
public class QnaUpdate implements Command {
	...
	public String exec(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			vo.setQnaNum(Integer.parseInt(multi.getParameter("qnaNum")));
			...
			vo.setQnaWriter((String) session.getAttribute("memberId"));
			...
			if (n > 0) {
				...
				out.println("alert('[수정완료] 글이 정상적으로 수정되었습니다.');location.href='adminQnaList.do';");
				out.println("</script>");
				...
				out.println("<script language='javascript'>");
				out.println("alert('[수정실패] 글을 정상적으로 수정하지 못했습니다.');location.href='adminQnaList.do';");
				out.println("</script>");
